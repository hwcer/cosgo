...
func (g *Group) Match(methods []string, path string, handler HandlerFunc, middleware ...MiddlewareFunc) []*RNode {
	routes := make([]*RNode, len(methods))
	for i, m := range methods {
		routes[i] = g.Add(m, path, handler, middleware...)
	}
...
func (g *Group) Add(method, path string, handler HandlerFunc, middleware ...MiddlewareFunc) *RNode {
