	...
	for !r.IsStop() {
		...
		if helper.null {
			...
			if atomic.CompareAndSwapInt32(&helper.init, 0, 1) {
				helper.udpMsgQue = newUdpAccept(r.conn, r.msgTyp, r.handler, addr)
				helper.null = false
			}
			helper.Unlock()
		}
		...
		if !helper.sendRead(data, n) {
			Logger.Error("drop msg because msgque full msgqueid:%v", helper.id)
		}
	}
}
...
func newUdpAccept(conn *net.UDPConn, msgtyp MsgType, handler IMsgHandler, addr *net.UDPAddr) *udpMsgQue {
	msgque := udpMsgQue{
		msgQue: msgQue{
	...
	Logger.Error("new msgque id:%d from addr:%s", msgque.id, addr.String())
	return &msgque
}

func newUdpListen(conn *net.UDPConn, msgtyp MsgType, handler IMsgHandler, addr string) *udpMsgQue {
	msgque := udpMsgQue{
		msgQue: msgQue{
			id:        atomic.AddUint32(&msgqueId, 1),
			msgTyp:    msgtyp,
	...
	msgqueMap[msgque.id] = &msgque
	msgqueMapSync.Unlock()
	Logger.Error("new udp listen id:%d addr:%s", msgque.id, addr)
	return &msgque
}
