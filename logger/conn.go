...
func NewNetAdapter(network, address string) *NetAdapter {
	return &NetAdapter{Network: network, Address: address}
}
...
func (c *NetAdapter) Write(msg *Message) (err error) {
...
func (c *NetAdapter) println(msg *Message) (err error) {
	c.Lock()
	defer c.Unlock()
	var txt string
	if c.Format != nil {
