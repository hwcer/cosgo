...
func ShowGCStat() {
	go func() {
		...
		gcstats := &rdebug.GCStats{PauseQuantiles: make([]time.Duration, 100)}
		memStats := &runtime.MemStats{}
		for {
			rdebug.ReadGCStats(gcstats)
			if gcstats.NumGC > numGC {
				runtime.ReadMemStats(memStats)

				printGC(memStats, gcstats, true)
				numGC = gcstats.NumGC
			}
			time.Sleep(interval)
		}
...
func GCSummary() string {
	...
	return printGC(memStats, gcstats, false)
}

func printGC(memStats *runtime.MemStats, gcstats *rdebug.GCStats, printStd bool) string {
	...
	if gcstats.NumGC > 0 {
		...
		result = fmt.Sprintf("NumGC:%d Pause:%s Pause(Avg):%s Overhead:%3.2f%% \nAlloc:%s Sys:%s Alloc(Rate):%s/s Histogram:%s %s %s",
			gcstats.NumGC,
			toS(lastPause),
		...
		result = fmt.Sprintf("Alloc:%s Sys:%s Alloc(Rate):%s/s\n",
			toH(memStats.Alloc),
			toH(memStats.Sys),
			toH(uint64(allocatedRate)))
	}
