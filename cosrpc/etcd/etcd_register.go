...
func (p *EtcdRegisterPlugin) Start() error {
	...
	if p.kv == nil {
		kv, err := libkv.NewStore(ETCD, p.EtcdServers, p.Options)
		if err != nil {
			return err
		}
		p.kv = kv
				...
				case <-ticker.C:
					...
					for _, name := range p.Services {
						...
						if err != nil {
							...
							if err != nil {
								logger.Error("cannot re-create etcd path %s: %v", nodePath, err)
							}
...
func (p *EtcdRegisterPlugin) Register(name string, rcvr interface{}, metadata string) (err error) {
	...
	if p.kv == nil {
		Register()
		kv, err := libkv.NewStore(ETCD, p.EtcdServers, nil)
		if err != nil {
			return err
		}
		p.kv = kv
