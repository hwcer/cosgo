...
// ETCDV3 backend
const ETCDV3 store.Backend = "etcdv3"
...
type EtcdV3 struct {
	...
	leaseKeepAliveStarted   int32
	leaseKeepAliveResponses <-chan *clientv3.LeaseKeepAliveResponse
}
...
func (s *EtcdV3) leaseKeepAliveStart() {
	if s.leaseKeepAliveStarted > 0 {
		return
	}
	if !atomic.CompareAndSwapInt32(&s.leaseKeepAliveStarted, 0, 1) {
		return
	}
...
func (s *EtcdV3) keepAlive() {
	for {
		...
		case <-s.done:
			return
		case resp := <-s.leaseKeepAliveResponses:
			//logger.Debug("etcd reconnect resp:%v", resp)
			if resp == nil { // connection is closed
				logger.Debug("租约已经失效了")
				s.client.Close()
				s.connect()
			} else {
				logger.Debug("收到自动续租应答 Index:%v  TTL:%v", s.leaseGrantResponse.ID, resp.TTL)
			}
		}
	}
}

func (s *EtcdV3) connect() {
	logger.Debug("EtcdV3 Connect")
	var err error
	for {
		select {
		case <-s.done:
		...
		default:
			if err = s.init(); err != nil {
				logger.Debug("EtcdV3 init err:%v", err)
				time.Sleep(time.Second)
				continue
			}
			if err = s.grant(); err != nil {
				logger.Debug("EtcdV3 Grant err:%v", err)
				time.Sleep(time.Second)
				continue
			}
			return
...
func (s *EtcdV3) grant() (err error) {
	ctx, cancel := context.WithTimeout(context.Background(), s.timeout)
	s.leaseGrantResponse, err = s.client.Grant(ctx, s.ttl)
	cancel()
	if err != nil {
		return err
	}
	logger.Debug("获得租约 Index:%v TTL:%v", s.leaseGrantResponse.ID, s.leaseGrantResponse.TTL)
	s.leaseKeepAliveResponses, err = s.client.KeepAlive(context.Background(), s.leaseGrantResponse.ID)
	return
}

// Put a value at the specified key
func (s *EtcdV3) Put(key string, value []byte, options *store.WriteOptions) error {
	s.leaseKeepAliveStart()
	ctx, cancel := context.WithTimeout(context.Background(), s.timeout)
	_, err := s.client.Put(ctx, key, string(value), clientv3.WithLease(s.leaseGrantResponse.ID))
	cancel()
	if err != nil {
		s.client.Close() //TODO 并发
		logger.Debug("ETCD3 PUT leaseID:%v, ERR :%v", s.leaseGrantResponse.ID, err)
	}
	return err
}
...
// Exists verifies if a Fields exists in the store
